...
async function findAuthorizedPermissionList(params){
    return request(`/api/system/staff-authorization/authorized-permission/list${stringify(params)}`,{
        method:'GET',
    })
}

async function findAuthorizedPermissionTree(params){
    return request(`/api/system/staff-authorization/authorized-permission/tree${stringify(params)}`,{
        method:'GET',
    })
}

async function findAuthorizedOrganizationList(params){
    return request(`/api/system/staff-authorization/authorized-organization/list${stringify(params)}`,{
        method:'GET',
    });
}

async function findAuthorizedOrganizationTree(params){
    return request(`/api/system/staff-authorization/authorized-organization/tree?${stringify(params)}`,{
        method:'GET',
    })
}
