...
import DataScopeEnum from '../../../enum/system/DataScopeEnum';
    ...
    return (
      <Modal {...modalConfig}>
        <Form>
          ...
          <FormItem {...formItemLayout} label="默认数据范围">
            {getFieldDecorator('dataScope', {
              rules: [{ required: false, message: '请选择岗位数据范围' }],
              initialValue: position.dataScope,
            })(
              <Select placeholder="默认访问所有数据" >
                {
                  DataScopeEnum.toList().map(key=>{
                    return <Option  value={key} key={key}>{DataScopeEnum.getDisplayText(key)}</Option>
                  })
                }
              </Select>
            )}
