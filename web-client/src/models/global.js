...
import staffFavoriteMenuService from "../services/system/StaffFavoriteMenuService";
  ...
  effects: {
    * initialize({payload}, {call, put, select}) {
      try {
        ...
        yield put({
          type: 'queryStaffFavoriteMenuList',
          payload: {}
        });

        yield put({
          type: 'queryStaffAuthorizedMenuList',
          payload: {}
        });

        yield put({
          type: 'queryAuthorizedPermissions',
          payload: {}
        });
    ...
    * queryAuthorizedPermissions({payload}, {put, call, select}) {
      const permissions = yield call(StaffAuthorizationService.findAuthorizedPermissionList);
      const authorizedPermissions = permissions.map((item) => {
    ...
    * queryStaffAuthorizedMenuList({payload}, {put, call, select}) {
      const currentModule = yield select(state=>state['global'].currentModule);
    ...
    * queryStaffFavoriteMenuList({payload}, {put, call, select}) {
      const favoriteMenus = yield call(staffFavoriteMenuService.findStaffFavoriteMenuList, payload);
    ...
    * addStaffFavoriteMenu({payload}, {put, call, select}) {

      yield call(staffFavoriteMenuService.addStaffFavoriteMenu, payload);
    ...
    * deleteStaffFavoriteMenuByMenuId({payload}, {put, call, select}) {

      yield call(staffFavoriteMenuService.deleteStaffFavoriteMenuByMenuId, payload);
